...
class BixiStationsRefresher
  ...
  def execute
    ...
    json_mapped_to_model_attrs = map_json_to_model_hash_attrs(json['stations'])
    ...
    array_of_stations_hash.map do |hash|
      {
        ...
        distance_to_fx_innovation: Geocoder::Calculations.distance_between(FAVORITE_LAT_LON[:fx_innovation], [hash['la'], hash['lo']]).round(3),
      }
    end
  end
end